...
class SentenceEncoder:
    ...
    def __init__(self, model_path, max_sentences=None, max_tokens=None, cpu=False, fp16=False, verbose=False,
                 sort_kind='quicksort', run_amp=False):
        self.use_cuda = torch.cuda.is_available() and not cpu
        self.run_amp = self.use_cuda and run_amp
        ...
        if self.use_cuda:
            ...
            self.encoder.cuda()
        if verbose:
            print(f' - Using AMP: {self.run_amp}')
    ...
    def _process_batch(self, batch):
        ...
        return embeddings.detach().cpu().numpy(), latency
    ...
    def encode_sentences(self, sentences):
        indices = []
        results = []
        total_gpu_latency = 0.0 if self.use_cuda else None

        for batch, batch_indices in self._make_batches(sentences):
            indices.extend(batch_indices)

            r, gpu_latency = self._process_batch(batch)
...
#        encoder.encode_sentences(sentences).tofile(out_file)
        ...
        embedding, gpu_latency = encoder.encode_sentences(sentences)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fp16', action='store_true',
                        help='Use model.half in order to speed up the inference')
    parser.add_argument('--run-amp', action='store_true',
                        help='Use Aumatic Mixed Precision')
    ...
    encoder = SentenceEncoder(args.encoder,
                              ...
                              cpu=args.cpu, fp16=args.fp16,
                              verbose=args.verbose, run_amp=args.run_amp)
